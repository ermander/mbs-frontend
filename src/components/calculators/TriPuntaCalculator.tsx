...
import { equalProfit, ratingPercent, stakeBCFromStakeA } from '@/lib/calculators/tri-punta'
...
export function TriPuntaCalculator() {
  ...
  const [quotaA, setQuotaA] = useState('3.00')
  const [quotaB, setQuotaB] = useState('3.50')
  const [quotaC, setQuotaC] = useState('4.00')
  ...
  const stakes = useMemo(() => {
    if (puntataEffettivaA <= 0 || quotaANum == null || quotaBNum == null || quotaCNum == null)
      return null
    return stakeBCFromStakeA(puntataEffettivaA, quotaANum, quotaBNum, quotaCNum)
  }, [puntataEffettivaA, quotaANum, quotaBNum, quotaCNum])
  ...
  const guadagnoMinimo = useMemo(() => {
    if (
      ...
      return null
    const base = equalProfit(puntataEffettivaA, quotaANum, stakeB, quotaBNum, stakeC, quotaCNum)
    if (base == null) return null
    return base + bonusNum
  }, [puntataEffettivaA, stakeB, stakeC, quotaANum, quotaBNum, quotaCNum, bonusNum])
  ...
  const realOutlay = (puntataANum ?? 0) + (stakeB ?? 0) + (stakeC ?? 0)
  const profitIfAWins =
    quotaANum != null && stakeB != null && stakeC != null
      ? puntataEffettivaA * quotaANum - realOutlay
      : null
  const profitIfBWins =
    stakeB != null && quotaBNum != null && stakeC != null ? stakeB * quotaBNum - realOutlay : null
  const profitIfCWins =
    stakeC != null && quotaCNum != null && stakeB != null ? stakeC * quotaCNum - realOutlay : null
              ...
              <div className="rounded-xl border border-white/10 bg-primary/10 p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-destructive">{formatSigned(-(stakeB ?? 0))} &euro;</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="rounded-xl border border-white/10 bg-primary/10 p-4">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Book B</span>
                    <span className="text-primary">
                      {formatSigned((returnB ?? 0) - (stakeB ?? 0))} &euro;
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Book C</span>
              ...
              <table className="w-full table-fixed text-sm">
                <thead>
                  <tr className="border-b border-white/10 text-muted-foreground">
                    ...
                    <th className="w-[13%] p-3 text-right font-normal">Book C</th>
                    <th className="w-[25%] min-w-[5.5rem] p-3 text-right font-normal">Totale</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr className="bg-primary/10">
                    ...
                    <td className="p-3 text-right text-primary">
                      {formatSigned((returnC ?? 0) - (stakeC ?? 0))}
                    </td>
                    <td className="min-w-[5.5rem] whitespace-nowrap p-3 text-right">
                      <span
              ...
              <div className="grid gap-4 px-6 py-5">
                ...
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    value={mercatoPuntaC}
                    onChange={(e) => setMercatoPuntaC(e.target.value)}
                    className="flex h-10 w-full appearance-none rounded-lg border border-input bg-background pl-3 pr-9 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:opacity-50 [&>option]:bg-background"
                  >
                    <option value="">Seleziona</option>
                    {MERCATI_TRI_PUNTA.map((m) => (
